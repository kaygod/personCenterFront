    ...
    <div class="container">
      <Form ref="formCustom" :label-width="80">
        ...
        >
          ...
          <p class="lt">
            <img :src="captcha" v-if="type == 1" style="height:30px" />
          </p>
        </FormItem>
        <FormItem>
...
export default {
  data() {
    return {
      ...
      captcha: `${service_ip}/api/user/getCaptcha`,
    };
  },
  computed: {
    // 计算属性的 getter
  ...
  methods: {
    ...
    handleSubmit() {
      const { user_name, password, code } = this.formCustom;
      let params = {
        user_name,
        password,
      };
      ...
      _axios.post(url, params).then((response) => {
        ...
        storeToken(token);
        this.$store.mutations.setUserInfo(token);
      });
    },
  },
};
